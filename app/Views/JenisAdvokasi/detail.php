...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page"><?= $result['nama_jenis_advokasi'] ?></li>
    </ol>
</nav>

<div class="d-flex align-items-center p-3 my-1 text-white-50 bg-info rounded shadow-sm">
    <div class="lh-100">
        <h6 class="mb-0 text-white lh-100 text-uppercase"><?= $result['nama_jenis_advokasi'] ?></h6>
    </div>
</div>
...
<div class="card mb-3">
    <div class="row">
        <div class="col-md-12">
            <div class="card-body">
                <h5 class="card-title"><?= $result['nama_jenis_advokasi'] ?></h5>
                <div class="row">
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card-body">
                <div class="d-flex align-items-center p-3 my-1 text-white-50 bg-info rounded shadow-sm">
                    <div class="lh-100">
                        <h6 class="mb-0 text-white lh-100">Grafik Layanan <?= $result['nama_jenis_advokasi'] ?></h6>
                    </div>
                </div>
                ...
                <div class="d-flex align-items-center p-3 my-1 text-white-50 bg-info rounded shadow-sm">
                    <div class="lh-100">
                        <h6 class="mb-0 text-white lh-100">Grafik Valuasi <?= $result['nama_jenis_advokasi'] ?></h6>
                    </div>
                </div>
                <div class="mb-3 p-3 bg-white rounded shadow-sm">
                    <form id="form-submit" action="<?= base_url('jenis-advokasi/' . $result['id']) ?>" method="get" class="mt-2 form-inline">
...
<script type="text/javascript">
    ...
    function drawChartLayanan() {
        var ajax = $.ajax({
            url: '<?= base_url("jenis-advokasi/chart/chart_layanan"); ?>',
        ...
        if(result.status){
            ...
            var options = {
                chart: {
                    title: 'Grafik Layanan <?= $result['nama_jenis_advokasi'] ?>',
                    subtitle: '<?= date('d-m-Y  h:i:s'); ?>'
                },
                colors: ['#FFAF2E','#FF7A20'],
                legend: { position: 'none' },
                vAxis: {
                    title: 'Layanan <?= $result['nama_jenis_advokasi'] ?>'
                },
                hAxis: {
                    title: 'Bulan',
                }
    ...
    function drawChartValuasi(){
        var ajax = $.ajax({
            url: '<?= base_url("jenis-advokasi/chart/chart_valuasi"); ?>',
        ...
        if(result.status){
            ...
            var options = {
                ...
                legend: { position: 'none' },
                vAxis: {
                    title: 'Valuasi <?= $result['nama_jenis_advokasi'] ?>',
                    format: '#,###'
                },
                hAxis: {
                    title: 'Bulan',
